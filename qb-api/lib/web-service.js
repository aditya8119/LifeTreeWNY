...
/**
 * A environment setup for process.env {username,password, qbfile}
 */
const envs = require('dotenv').config();
// console.log(process.env.QB_USERNAME);
// console.log(process.env.QB_PASSWORD);
// console.log(process.env.QB_COMPANY_FILE);
...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.authenticate = function(args, callback) {
    ...
    qbXMLHandler.updateSessionId(authReturn[0]);
...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.sendRequestXML = function(args, callback) {
    ...
    console.log(chalk.green('WebMethod: sendRequestXML() has been called by QBWebConnector'));
    callback({
...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.receiveResponseXML = function(args, callback) {
    ...
    if (hresult) {
        ...
        if ((typeof qbXMLHandler.didReceiveError === "function")) {
            qbXMLHandler.didReceiveError(hresult, counter - 1);
        }
    } else {
        if ((typeof qbXMLHandler.handleResponse === "function")) {
            qbXMLHandler.handleResponse(response, counter - 1);
        }
    ...
    console.log('        Number retVal = ' + retVal);
    callback ({
