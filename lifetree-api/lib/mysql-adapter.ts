...
class MysqlAdapter {
  ...
  public selectAddress(): Promise<any> {
    return this.knex.raw('SELECT concat(`Shipping Address Line 1`, `Shipping Address Line 2`) as address, `Shipping Address City` as city, `Shipping Address State`as state, `Shipping Address Postal Code` as pincode, `First Name` as firstName , `Last Name` as lastName, `Total` as total FROM `customers`.`proposals` limit 0,10;').then((rows: any) => {
      return rows;
    }, (error: any) => {
      return error;
    });
